...
type UserRole="owner"|"manager"|"accountant"|"cashier"|"waiter"|"barista"|"employee"|"custom";
type UserPermission=View|"users";
type SystemUser={id:string;name:string;role:UserRole;title:string;branch:string;phone:string;email:string;permissions:UserPermission[];active:boolean};
...
const allPermissions:UserPermission[]=[...nav.map(item=>item.id),"users"];
const roleOptions:{id:UserRole;label:string;title:string;permissions:UserPermission[]}[]=[
  {id:"owner",label:"المالك Owner",title:"مالك",permissions:allPermissions},
  {id:"manager",label:"المدير",title:"مدير المقهى",permissions:["live","compose","floor","recipes","prep","batches","waste","guests","insights"]},
  {id:"accountant",label:"المحاسب",title:"محاسب",permissions:["batches","finance","tax","insights"]},
  {id:"cashier",label:"موظف الكاشير",title:"كاشير",permissions:["live","compose","floor","guests"]},
  {id:"waiter",label:"مقدم الخدمة Waiter",title:"مقدم خدمة",permissions:["live","floor","guests"]},
  {id:"barista",label:"صانع القهوة",title:"باريستا",permissions:["live","compose","recipes","prep","waste"]},
  {id:"employee",label:"موظف",title:"موظف",permissions:["live"]},
  {id:"custom",label:"مسمى مخصص",title:"",permissions:[]},
];
const roleDetails=(role:UserRole)=>roleOptions.find(item=>item.id===role)??roleOptions[7];
const completeUser=(user:Partial<SystemUser>,index:number):SystemUser=>{
  const legacyRole=String(user.role??"");
  const inferred:UserRole=legacyRole==="owner"||legacyRole.includes("النظام")?"owner":legacyRole==="manager"||legacyRole.includes("الوردية")?"manager":legacyRole==="accountant"||legacyRole.includes("محاسب")?"accountant":legacyRole==="barista"||legacyRole.includes("التحضير")?"barista":legacyRole==="waiter"?"waiter":legacyRole==="employee"?"employee":"cashier";
  const preset=roleDetails(inferred);
  return {id:user.id??`USR-${index+1}`,name:user.name??"",role:inferred,title:user.title??preset.title,branch:user.branch??"دبي",phone:user.phone??"",email:user.email??"",permissions:Array.isArray(user.permissions)&&user.permissions.length?user.permissions:preset.permissions,active:user.active!==false};
};
...
const defaultSystemSettings:SystemSettings={
  ...
  theme:"orange",
  users:[
    {id:"USR-1",name:"أريج علوش",role:"owner",title:"مالك",branch:"دبي",phone:"050 000 1201",email:"",permissions:allPermissions,active:true},
    {id:"USR-2",name:"سامي حداد",role:"manager",title:"مدير المقهى",branch:"دبي",phone:"050 000 1202",email:"",permissions:roleDetails("manager").permissions,active:true},
    {id:"USR-3",name:"رامي ناصر",role:"cashier",title:"كاشير",branch:"دبي",phone:"050 000 1203",email:"",permissions:roleDetails("cashier").permissions,active:true},
  ],
};
...
function Icon({ name, size = 20 }: { name: string; size?: number }) {
  ...
  switch(name){
    ...
    case "power": return <svg {...p}><path d="M12 2v10"/><path d="M18.4 6.6a9 9 0 1 1-12.8 0"/></svg>;
...
export default function Home(){
  ...
  const [activeUserId,setActiveUserId]=useState(defaultSystemSettings.users[0].id);
  const [userSwitcher,setUserSwitcher]=useState(false);

  useEffect(()=>{let active=true;fetch("/api/cafe-settings",{cache:"no-store"}).then(async response=>{if(!response.ok)throw new Error("settings");return response.json() as Promise<SystemSettings>}).then(saved=>{if(!active)return;const theme=themes.some(item=>item.id===saved.theme)?saved.theme:"orange";const users=(Array.isArray(saved.users)?saved.users:defaultSystemSettings.users).map(completeUser);setSystemSettings({...defaultSystemSettings,...saved,business:{...defaultSystemSettings.business,...saved.business},operations:{...defaultSystemSettings.operations,...saved.operations},theme,users});const remembered=localStorage.getItem("varex_cafe_active_user");setActiveUserId(users.some(user=>user.id===remembered&&user.active)?remembered!:users.find(user=>user.active)?.id??users[0].id)}).catch(()=>{});return()=>{active=false}},[]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    document.querySelectorAll<HTMLLinkElement>('link[rel~="icon"],link[rel="apple-touch-icon"]').forEach(node=>node.href=icon);
    const manifest=document.querySelector<HTMLLinkElement>('link[rel="manifest"]');
    if(manifest)manifest.href=`/api/cafe-manifest?theme=${encodeURIComponent(theme)}`;
    const themeMeta=document.querySelector<HTMLMetaElement>('meta[name="theme-color"]');
    if(themeMeta)themeMeta.content=(themes.find(item=>item.id===theme)??themes[0]).color;
    try{localStorage.setItem("varex_cafe_appearance",theme)}catch{}
  },[systemSettings.theme]);

  const activeUser=systemSettings.users.find(user=>user.id===activeUserId&&user.active)??systemSettings.users[0];
  const visibleNav=activeUser.role==="owner"?nav:nav.filter(item=>item.id==="settings"?activeUser.permissions.includes("settings")||activeUser.permissions.includes("users"):activeUser.permissions.includes(item.id));
  const current=visibleNav.find(x=>x.id===view)??visibleNav[0]??nav[0];
  const activeInitials=activeUser.name.split(" ").filter(Boolean).map(part=>part[0]).join("").slice(0,2).toUpperCase()||"VA";
  const appearance=themes.find(item=>item.id===systemSettings.theme)??themes[0];
  useEffect(()=>{if(!visibleNav.some(item=>item.id===view))setView(visibleNav[0]?.id??"live")},[activeUserId,systemSettings.users,view]);
  ...
  function activateUser(user:SystemUser){const allowed=user.role==="owner"?nav:nav.filter(item=>item.id==="settings"?user.permissions.includes("settings")||user.permissions.includes("users"):user.permissions.includes(item.id));const first=allowed[0]?.id??"live";setActiveUserId(user.id);localStorage.setItem("varex_cafe_active_user",user.id);setView(first);setUserSwitcher(false);notify(`تم التبديل إلى ${user.name}`)}
  ...
  return <div className="cafe-os" style={{"--ember":appearance.color} as CSSProperties}>
    ...
    {userSwitcher&&<div className="modal-layer user-switcher-layer" role="dialog" aria-modal="true" aria-label="تبديل المستخدم"><section className="user-switcher-modal"><header><span><Icon name="switch" size={24}/></span><div><small>جلسة العمل</small><h2>تبديل المستخدم</h2><p>اختيار الحساب المطلوب لعرض الأقسام المسموح بها فقط.</p></div></header><div className="switch-user-list">{systemSettings.users.filter(user=>user.active).map(user=><button key={user.id} className={user.id===activeUser.id?"active":""} onClick={()=>confirm({title:"تبديل المستخدم",message:`هل تريد الانتقال إلى حساب ${user.name} بصلاحية ${user.title}؟`,confirmLabel:"تأكيد التبديل",onConfirm:()=>activateUser(user)})}><span>{user.name.split(" ").filter(Boolean).map(part=>part[0]).join("").slice(0,2).toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.title} · {user.branch}</small></div><b>{user.role==="owner"?"كل الصلاحيات":`${user.permissions.length} أقسام`}</b></button>)}</div><footer><button onClick={()=>setUserSwitcher(false)}>إغلاق</button></footer></section></div>}
    <header className="rail">
      <button type="button" className="varex-brand" onClick={()=>setView("live")}>
        <span className="approved-varex-icon"><img className="approved-varex-logo" src="/varex-approved-transparent-512.png" alt=""/></span>
        <span className="brand-copy"><b>VAREX</b><small>BUSINESS MANAGEMENT SYSTEM</small></span>
      </button>
      <div className="cafe-identity"><i>مقهى الأريج</i><strong>{systemSettings.business.name}</strong><span><em/> فرع {systemSettings.business.branch} · الوردية مفتوحة</span></div>
      <nav>{visibleNav.map(item=><button key={item.id} className={view===item.id?"active":""} onClick={()=>setView(item.id)}><span><Icon name={item.icon}/></span><b>{item.label}<small>{item.caption}</small></b></button>)}</nav>
      <div className="rail-bottom"><button className="owner"><span>{activeInitials}</span><b><strong>{activeUser.name}</strong><small>{activeUser.title}</small></b></button><div className="account-actions"><button className="switch-user" disabled={loggingOut} onClick={()=>setUserSwitcher(true)}><Icon name="switch" size={23}/><span>تبديل المستخدم</span></button><button className="logout" disabled={loggingOut} onClick={()=>confirm({title:"تسجيل الخروج",message:"هل تريد إنهاء الجلسة الحالية وتسجيل الخروج؟",confirmLabel:"تسجيل الخروج",onConfirm:()=>{void logout()}})}><Icon name="power" size={25}/><span>{loggingOut?"جاري تسجيل الخروج...":"تسجيل الخروج"}</span></button></div><p>VAREX Café v1.0</p></div>
    </header>
    ...
    <main className="stage">
      <header className="shift-bar">
        ...
        <div className="shift-actions">
          <button onClick={()=>confirm({title:"لغة النظام",message:"هل تريد فتح خيارات لغة النظام؟",confirmLabel:"فتح اللغات",onConfirm:()=>setView("settings")})}>{systemSettings.business.language} <span>{languages.find(item=>item.name===systemSettings.business.language)?.code??"AR"}</span></button>
      ...
      <section className="page-intro">
        ...
        {view!=="compose"&&(activeUser.role==="owner"||activeUser.permissions.includes("compose"))&&<button className="compose-cta" onClick={()=>setDrinkModal(true)}><Icon name="spark"/><span>إنشاء مشروب</span><small>وصفة جديدة للمقهى</small></button>}
      </section>
      ...
      {view==="settings"&&<Settings confirm={confirm} notify={notify} settings={systemSettings} setSettings={setSystemSettings} saveSettings={saveSystemSettings} saving={settingsSaving} savingLabel={settingsSavingLabel} canManageSystem={activeUser.role==="owner"||activeUser.permissions.includes("settings")} canManageUsers={activeUser.role==="owner"||activeUser.permissions.includes("users")}/>}
    </main>
  </div>
}
...
function Settings({confirm,notify,settings,setSettings,saveSettings,saving,savingLabel,canManageSystem,canManageUsers}:{confirm:ConfirmAction;notify:(x:string)=>void;settings:SystemSettings;setSettings:React.Dispatch<React.SetStateAction<SystemSettings>>;saveSettings:(next?:SystemSettings,message?:string,busyLabel?:string)=>Promise<boolean>;saving:boolean;savingLabel:string;canManageSystem:boolean;canManageUsers:boolean}){
  ...
  const emptyUser:SystemUser={id:"",name:"",role:"cashier",title:"كاشير",branch:"دبي",phone:"",email:"",permissions:roleDetails("cashier").permissions,active:true};
  ...
  function applyUserRole(role:UserRole){const preset=roleDetails(role);setUserDraft(value=>({...value,role,title:preset.title||value.title,permissions:role==="owner"?allPermissions:preset.permissions}))}
  function togglePermission(permission:UserPermission){if(userDraft.role==="owner")return;setUserDraft(value=>({...value,permissions:value.permissions.includes(permission)?value.permissions.filter(item=>item!==permission):[...value.permissions,permission]}))}
  function saveUser(e:React.FormEvent){e.preventDefault();if(!userDraft.name.trim()||!userDraft.title.trim()){notify("اسم المستخدم والمسمى الوظيفي مطلوبان");return}if(userDraft.role!=="owner"&&!userDraft.permissions.length){notify("يلزم اختيار صلاحية واحدة على الأقل");return}confirm({title:userIndex===null?"إضافة مستخدم":"تعديل المستخدم",message:`هل تريد حفظ بيانات ${userDraft.name} وصلاحيات ${userDraft.title}؟`,confirmLabel:"حفظ المستخدم",onConfirm:()=>{const value={...userDraft,id:userDraft.id||crypto.randomUUID(),permissions:userDraft.role==="owner"?allPermissions:userDraft.permissions};const updated=userIndex===null?[...users,value]:users.map((item,index)=>index===userIndex?value:item);const next={...settings,users:updated};setSettings(next);void saveSettings(next,"تم حفظ المستخدم وتطبيق صلاحيات الأقسام","جاري الحفظ...").then(ok=>{if(ok){setUserIndex(null);setUserDraft(emptyUser);setEditor(null)}})}})}
  function removeUser(){if(userIndex===null)return;const current=users[userIndex];if(current.role==="owner"){notify("لا يمكن حذف حساب المالك");return}confirm({title:"حذف المستخدم",message:`هل تريد حذف حساب ${current.name} وإيقاف الوصول إلى النظام؟`,confirmLabel:"تأكيد الحذف",onConfirm:()=>{const next={...settings,users:users.filter((_,index)=>index!==userIndex)};setSettings(next);setUserIndex(null);setUserDraft(emptyUser);void saveSettings(next,"تم حذف المستخدم وإيقاف الصلاحيات")}})}
  return <>
    {canManageSystem&&editor==="business"&&<div className="modal-layer settings-editor-layer"><form className="command-modal settings-command" onSubmit={saveBusiness}><header><span><Icon name="settings"/></span><div><small>بيانات النشاط</small><h2>تعديل بيانات المقهى</h2><p>الهوية والفرع والبيانات الضريبية ووسائل التواصل.</p></div></header><div className="command-grid"><label>اسم النشاط<input required value={businessDraft.name} onChange={e=>setBusinessDraft(v=>({...v,name:e.target.value}))}/></label><label>الفرع<input required value={businessDraft.branch} onChange={e=>setBusinessDraft(v=>({...v,branch:e.target.value}))}/></label><label>العملة<select value={businessDraft.currency} onChange={e=>setBusinessDraft(v=>({...v,currency:e.target.value}))}><option>درهم إماراتي</option><option>ريال سعودي</option><option>دولار أمريكي</option></select></label><label>لغة التشغيل<select value={businessDraft.language} onChange={e=>setBusinessDraft(v=>({...v,language:e.target.value}))}>{languages.map(language=><option key={language.code} value={language.name}>{language.name}</option>)}</select></label><label>الرقم الضريبي TRN<input value={businessDraft.trn} onChange={e=>setBusinessDraft(v=>({...v,trn:e.target.value}))}/></label><label>رقم التواصل<input value={businessDraft.phone} onChange={e=>setBusinessDraft(v=>({...v,phone:e.target.value}))}/></label></div><footer><button type="button" className="secondary" disabled={saving} onClick={()=>setEditor(null)}>إلغاء</button><button type="submit" disabled={saving}>{saving?savingLabel:"حفظ البيانات"}</button></footer></form></div>}
    {canManageUsers&&editor==="users"&&<div className="modal-layer settings-editor-layer"><form className="command-modal settings-command user-permissions-command" onSubmit={saveUser}><header><span><Icon name="guests"/></span><div><small>الوصول والأمان</small><h2>إدارة المستخدمين والصلاحيات</h2><p>إنشاء حساب وتحديد المسمى والأقسام التي يمكن الوصول إليها.</p></div></header><div className="user-editor-list">{users.map((user,index)=><button type="button" className={userIndex===index?"active":""} key={user.id} onClick={()=>selectUser(index)}><span>{user.name}</span><small>{roleDetails(user.role).label} · {user.title}</small></button>)}<button type="button" className={userIndex===null?"active add":"add"} onClick={()=>{setUserIndex(null);setUserDraft(emptyUser)}}><Icon name="plus"/>حساب جديد</button></div><div className="command-grid"><label>اسم المستخدم<input required value={userDraft.name} onChange={e=>setUserDraft(v=>({...v,name:e.target.value}))}/></label><label>الدور الأساسي<select value={userDraft.role} onChange={e=>applyUserRole(e.target.value as UserRole)}>{roleOptions.map(role=><option key={role.id} value={role.id}>{role.label}</option>)}</select></label><label>المسمى الوظيفي<input required placeholder="مثال: مشرف صالة" value={userDraft.title} onChange={e=>setUserDraft(v=>({...v,title:e.target.value}))}/></label><label>البريد الإلكتروني<input type="email" value={userDraft.email} onChange={e=>setUserDraft(v=>({...v,email:e.target.value}))}/></label><label>رقم التواصل<input value={userDraft.phone} onChange={e=>setUserDraft(v=>({...v,phone:e.target.value}))}/></label><label>الفرع<input value={userDraft.branch} onChange={e=>setUserDraft(v=>({...v,branch:e.target.value}))}/></label><label>حالة الحساب<select value={userDraft.active?"active":"inactive"} onChange={e=>setUserDraft(v=>({...v,active:e.target.value==="active"}))}><option value="active">نشط</option><option value="inactive">موقوف</option></select></label><fieldset className="permissions-field"><legend>صلاحيات الأقسام</legend><p>تحديد ما يظهر لهذا الحساب. الأرباح والحسابات لا تظهر إلا عند اختيارها.</p><div className="permissions-grid">{nav.map(item=>{const checked=userDraft.role==="owner"||userDraft.permissions.includes(item.id);return <label className={checked?"selected":""} key={item.id}><input type="checkbox" checked={checked} disabled={userDraft.role==="owner"} onChange={()=>togglePermission(item.id)}/><span><Icon name={item.icon} size={17}/></span><b>{item.label}</b><small>{checked?"مسموح":"محجوب"}</small></label>})}{(()=>{const checked=userDraft.role==="owner"||userDraft.permissions.includes("users");return <label className={checked?"selected":""}><input type="checkbox" checked={checked} disabled={userDraft.role==="owner"} onChange={()=>togglePermission("users")}/><span><Icon name="guests" size={17}/></span><b>إدارة المستخدمين</b><small>{checked?"مسموح":"محجوب"}</small></label>})()}</div></fieldset></div><footer>{userIndex!==null&&userDraft.role!=="owner"&&<button type="button" className="danger" disabled={saving} onClick={removeUser}>حذف المستخدم</button>}<button type="button" className="secondary" disabled={saving} onClick={()=>setEditor(null)}>إلغاء</button><button type="submit" disabled={saving}>{saving?savingLabel:userIndex===null?"إضافة المستخدم":"حفظ التعديل"}</button></footer></form></div>}
    <div className="settings-page">
    {canManageSystem&&<section className="settings-card identity-settings"><header><span><Icon name="settings"/></span><div><small>بيانات النشاط</small><h2>{business.name}</h2><p>فرع {business.branch} · مقهى الأريج · الوردية المسائية</p></div></header><div className="settings-fields"><label>اسم النشاط<input value={business.name} readOnly/></label><label>الفرع<input value={business.branch} readOnly/></label><label>العملة<input value={business.currency} readOnly/></label><label>لغة التشغيل<input value={business.language} readOnly/></label></div><button onClick={()=>{setBusinessDraft(business);setEditor("business")}}>تعديل بيانات المقهى</button></section>}
    {canManageSystem&&<section className="settings-card operation-settings"><header><span><Icon name="sliders"/></span><div><small>الأوامر والمحطات</small><h2>إعدادات التشغيل</h2><p>الطباعة والتنبيهات ومسار تنفيذ الطلب.</p></div></header><div className="setting-switches">{settingRows.map(item=><button key={item.id} onClick={()=>requestToggle(item)}><span><strong>{item.title}</strong><small>{item.note}</small></span><i className={options[item.id]?"on":""}><em/></i></button>)}</div></section>}
    {canManageUsers&&<section className="settings-card access-settings"><header><span><Icon name="guests"/></span><div><small>الوصول والأمان</small><h2>المستخدمون والصلاحيات</h2><p>إدارة أدوار فريق المقهى والجلسات النشطة.</p></div></header><div className="access-summary"><div><strong>{users.length}</strong><span>مستخدمون</span></div><div><strong>{new Set(users.map(user=>user.role)).size}</strong><span>أدوار تشغيل</span></div><div><strong>{users.filter(user=>user.active).length}</strong><span>حسابات نشطة</span></div></div><button onClick={openUsers}>إدارة المستخدمين</button><button className="secondary" disabled={saving} onClick={()=>confirm({title:"حفظ الإعدادات",message:"هل تريد حفظ بيانات المقهى والمظهر وخيارات التشغيل والمستخدمين وربطها بالنظام؟",confirmLabel:"حفظ الكل",onConfirm:()=>{void saveSettings(settings)}})}>{saving?savingLabel:"حفظ الإعدادات"}</button></section>}
  </div>
  </>
}
