(function(){
  ...
  const SCALE_RATIOS={50:.70,60:.76,70:.82,80:.88,90:.94,100:1};
  const PREVIEW_MODE=new URLSearchParams(location.search).get("preview")==="1"||(location.hostname==="app.varexapp.com"&&location.pathname.startsWith("/restaurant/"));
  let previewSubscription=null,previewSelectedPlan=null;
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      *,*::before,*::after{scroll-behavior:auto!important;animation:none!important;transition:none!important}
      ...
      .vx-scale-presets{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-top:10px}
  ...
  function bindScale(options={}){
    ...
    let scale=PREVIEW_MODE?100:Number(localStorage.getItem(key));
    ...
    const apply=value=>{
      ...
      const ratio=SCALE_RATIOS[scale]||1;
      target.style.zoom="1";
      ...
      if(!PREVIEW_MODE)localStorage.setItem(key,String(scale));
  ...
  function subscriptionState(){
    if(PREVIEW_MODE)return {subscription:previewSubscription,selected:previewSelectedPlan};
  ...
  function bindSubscription(options={}){
    ...
    document.querySelectorAll("[data-vx-plan]").forEach(button=>button.addEventListener("click",()=>{
      ...
      if(PREVIEW_MODE){
        const plan={monthly:["monthly",t("الخطة الشهرية","Monthly Plan"),179,"month"],annual:["annual",t("الخطة السنوية","Annual Plan"),1799,"year"],lifetime:["lifetime",t("ترخيص مدى الحياة","Lifetime License"),5999,"lifetime"]}[id];if(!plan)return;
        previewSelectedPlan={plan:plan[0],planName:plan[1],price:plan[2],currency:"AED",billingType:plan[3],selectedAt:new Date().toISOString(),preview:true};
        rerender();toast(t("تمت محاكاة اختيار الخطة ببيانات مؤقتة — لم يتم تنفيذ أي دفع حقيقي","Plan selection simulated with temporary data — no real payment was made"));return;
      }
      if(id==="trial"){
  ...
  function boot(){
    ...
    const requestedLanguage=new URLSearchParams(location.search).get("lang"),language=PREVIEW_MODE?(requestedLanguage==="ar"?"ar":"en"):(localStorage.getItem("varex_language")||localStorage.getItem("varex_launcher_language"))==="en"?"en":"ar",t=(ar,en)=>language==="en"?en:ar;
    ...
    if(config.disableScale){if(!PREVIEW_MODE)localStorage.removeItem(config.key);shell.style.removeProperty("zoom");shell.style.removeProperty("width");shell.style.removeProperty("min-height");document.getElementById("vxScaleWrap")?.remove()}else if(!shell.dataset.vxScaleReady){shell.dataset.vxScaleReady="1";const haptic=document.querySelector(config.haptic);(haptic||top.firstElementChild)?.insertAdjacentHTML(haptic?"beforebegin":"beforebegin",scaleControl({t,buttonClass:config.button,icon:screenIcon,accent:config.accent}));bindScale({targetId:shell.id||(shell.id="vxAppShell"),storageKey:config.key})}
    // Subscription purchase now lives before the application opens, so it is
    // intentionally not exposed as an internal navigation item.
