...
let x=String(v).trim().replace(/[,\s]/g,"").replace(/[^\d.\-]/g,"");
const first=x.indexOf(".");
if(first!==-1)x=x.slice(0,first+1)+x.slice(first+1).replace(/\./g,"");
const n=Number(x);
...
const type=String(options.billingType||options.type||"monthly"),now=new Date();
...
"amount","total","subtotal","grandtotal","nettotal","finaltotal","price","cost","paid","payment","balance","revenue","income","expense","profit","purchase","sales","salary","debt","credit","cash","vat","tax",
"مبلغ","اجمالي","إجمالي","المجموع","السعر","سعر","تكلفة","التكلفة","مدفوع","المدفوع","دفعة","الدفع","رصيد","الرصيد","إيراد","ايراد","مبيعات","مشتريات","مصروف","مصاريف","ربح","أرباح","ارباح","راتب","رواتب","دين","ديون","نقدي","ضريبة"
];

function varexIsFinancialElement(el){
if(!el||el.nodeType!==1)return false;

if(el.matches("[data-varex-money],[data-money],[data-currency],[data-amount]"))return true;
...
if(VAREX_FINANCIAL_WORDS.some(w=>source.includes(String(w).toLowerCase())))return true;

const parentText=(el.parentElement?.textContent||"").slice(0,180).toLowerCase();

return VAREX_FINANCIAL_WORDS.some(w=>parentText.includes(String(w).toLowerCase()))
}
...
if(
/^\d{1,4}[\/\-]\d{1,2}[\/\-]\d{1,4}/.test(original)||
/^\d{1,2}:\d{2}/.test(original)
)return null;
...
const numeric=VAREX.parseNumber(raw,NaN);
...
if(
el.matches("input,textarea,select,option,script,style")||
el.closest("script,style")
)return;

if(!varexIsFinancialElement(el))return;

if(el.children.length>0){
Array.from(el.children).forEach(child=>varexFormatFinancialElement(child));
return
}

const data=varexExtractFinancialText(el.textContent);

if(!data)return;

const formatted=VAREX.formatNumber(data.numeric,2);

if(data.raw===formatted)return;

el.textContent=data.original.replace(data.raw,formatted);

el.dataset.varexNumberFormatted="true"
}
...
try{
if(root.nodeType===1&&root.matches?.(selectors))varexFormatFinancialElement(root);

(root.querySelectorAll?.(selectors)||[]).forEach(varexFormatFinancialElement)
}catch(e){}
}
...
if(varexFinancialObserver)return;

varexFinancialObserver=new MutationObserver(mutations=>{
clearTimeout(varexFinancialTimer);

varexFinancialTimer=setTimeout(()=>{
for(const mutation of mutations){

if(mutation.type==="characterData"){
const parent=mutation.target.parentElement;
if(parent)varexFormatFinancialNumbers(parent)
}

if(mutation.type==="childList"){
mutation.addedNodes.forEach(node=>{
if(node.nodeType===1)varexFormatFinancialNumbers(node)
})
}
...
varexFinancialObserver.observe(document.body,{
subtree:true,
childList:true,
characterData:true
})
...
/* ========================================================= GLOBAL TYPOGRAPHY - MAIN + SUB ========================================================= */

function varexGetTypography(){
let t={};
try{t=JSON.parse(localStorage.getItem("varexTypography")||"{}")||{}}catch(e){}

const oldFamily=t.fontFamily||"Arial,Tahoma,sans-serif";
const oldSize=Math.max(8,Math.min(36,Number(t.fontSize||13)));

return{
mainFontFamily:t.mainFontFamily||oldFamily,
mainFontSize:Math.max(12,Math.min(48,Number(t.mainFontSize||18))),
subFontFamily:t.subFontFamily||oldFamily,
subFontSize:Math.max(8,Math.min(36,Number(t.subFontSize||oldSize))),
bold:!!t.bold,
italic:!!t.italic,
underline:!!t.underline,
align:["right","center","left"].includes(t.align)?t.align:"right",
verticalAlign:["top","center","bottom"].includes(t.verticalAlign)?t.verticalAlign:"center"
}
}
...
main=t.mainFontSize,
sub=t.subFontSize,
small=Math.max(8,sub-2),
tiny=Math.max(8,sub-3),
hero=Math.min(58,main+7),
page=Math.min(52,main+2),
section=Math.min(48,main),
card=Math.max(12,main-2),
vertical=t.verticalAlign==="top"?"top":t.verticalAlign==="bottom"?"bottom":"middle";

const r=document.documentElement.style;

r.setProperty("--varex-main-font",t.mainFontFamily);
r.setProperty("--varex-main-size",main+"px");
r.setProperty("--varex-sub-font",t.subFontFamily);
r.setProperty("--varex-sub-size",sub+"px");
r.setProperty("--varex-font-align",t.align);
r.setProperty("--varex-font-valign",t.verticalAlign);
...
s.textContent=`
body,input,select,textarea,button,table{font-family:${t.subFontFamily}!important}
body{font-size:${sub}px!important}
p,label,input,select,textarea,button,td,th,.nav-label,.system-row,.switch-row,.field,.activity-text,.activity-text span,.module p,.panel p,.card p,.info-chip,.chip,.language-button,.sidebar .nav a,.varex-sidebar-actions button,.modal-content,.modal-box,.summary,.note,.badge,.status,.filter,.filters{font-family:${t.subFontFamily}!important;font-size:${sub}px!important}
small,.muted,.stat-small,.activity-time,.page-name small,.brand-small,.sidebar-footer,.module small,.card small,.panel small{font-family:${t.subFontFamily}!important;font-size:${small}px!important}
h1,h2,h3,h4,h5,h6,.hero h1,.page-name h2,.panel h2,.card h2,.card h3,.module h3,.section-header h3,.stat-value,.summary-title,.modal-title{font-family:${t.mainFontFamily}!important}
h1,.hero h1{font-size:${hero}px!important}
h2,.page-name h2{font-size:${page}px!important}
.panel h2,.card h2,.section-header h3{font-size:${section}px!important}
h3,.card h3,.module h3,.stat-value{font-size:${card}px!important}
.logo,.varex-logout-brand,.varex-transition-word{font-family:${t.mainFontFamily}!important}
td,th{vertical-align:${vertical}!important}
.varex-user-align{text-align:${t.align}!important}
body.varex-font-bold h1,body.varex-font-bold h2,body.varex-font-bold h3,body.varex-font-bold h4,body.varex-font-bold h5,body.varex-font-bold h6,body.varex-font-bold p,body.varex-font-bold label,body.varex-font-bold td,body.varex-font-bold th,body.varex-font-bold input,body.varex-font-bold select,body.varex-font-bold textarea{font-weight:700!important}
body.varex-font-italic h1,body.varex-font-italic h2,body.varex-font-italic h3,body.varex-font-italic p,body.varex-font-italic label,body.varex-font-italic td,body.varex-font-italic th{font-style:italic!important}
body.varex-font-underline h1,body.varex-font-underline h2,body.varex-font-underline h3,body.varex-font-underline p,body.varex-font-underline label,body.varex-font-underline td,body.varex-font-underline th{text-decoration:underline!important}
.varex-sub-tiny{font-size:${tiny}px!important}
`;
...
document.querySelectorAll(".panel p,.card p,.module p,.activity-text,.activity-text span,.system-row,tbody td,.summary,.note,.description,.details").forEach(e=>e.classList.add("varex-user-align"))
}
...
if(!s)return;
const v=String(s.scrollTop||0);
try{
sessionStorage.setItem(VAREX_SIDEBAR_SCROLL_KEY,v);
localStorage.setItem(VAREX_SIDEBAR_SCROLL_KEY,v)
}catch(e){}
}
...
let v=0;
try{
v=Number(sessionStorage.getItem(VAREX_SIDEBAR_SCROLL_KEY)||localStorage.getItem(VAREX_SIDEBAR_SCROLL_KEY)||0)
}catch(e){}
if(!Number.isFinite(v)||v<0)v=0;
const restore=()=>{s.scrollTop=Math.min(v,Math.max(0,s.scrollHeight-s.clientHeight))};
...
let current=location.pathname.split("/").pop().toLowerCase()||"index.html";

if(current==="subscription-success.html")current="subscription.html";

nav.innerHTML=VAREX_MENU.map(([file,icon,title])=>
`<a href="./${file}" class="${current===file.toLowerCase()?"active":""}">
...
<div class="varex-logout-brand">VAREX</div>
...
<p class="varex-logout-message" id="varexLogoutMessage">هل تريد تسجيل الخروج من VAREX؟</p>
<div class="varex-logout-progress" id="varexLogoutProgress"><div class="varex-logout-progress-bar" id="varexLogoutProgressBar"></div></div>
<div class="varex-logout-actions" id="varexLogoutActions">
<button type="button" class="varex-logout-confirm" id="varexLogoutConfirm"><span>⏻</span><span>تأكيد تسجيل الخروج</span></button>
<button type="button" class="varex-logout-cancel" id="varexLogoutCancel"><span>↩</span><span>البقاء في النظام</span></button>
</div>
</div>
<div class="varex-logout-transition" id="varexLogoutTransition"><div class="varex-transition-word">VAREX</div></div>
`;
...
if(e.target===o&&!o.classList.contains("processing"))varexCloseLogoutDialog()
})
}
...
o.classList.remove("processing","finished","leaving");

document.getElementById("varexLogoutTransition")?.classList.remove("show");

const t=document.getElementById("varexLogoutTitle"),
m=document.getElementById("varexLogoutMessage"),
a=document.getElementById("varexLogoutActions"),
l=document.getElementById("varexLogoutLoader"),
s=document.getElementById("varexLogoutSuccessIcon"),
p=document.getElementById("varexLogoutProgress"),
b=document.getElementById("varexLogoutProgressBar");

if(t)t.textContent="تسجيل الخروج";
if(m)m.textContent="هل تريد تسجيل الخروج من VAREX؟";
...
const o=document.getElementById("varexLogoutOverlay");
if(!o||o.classList.contains("processing"))return;
...
function varexSetLogoutStatus(t,m,p,mode="loading"){
const T=document.getElementById("varexLogoutTitle"),
M=document.getElementById("varexLogoutMessage"),
L=document.getElementById("varexLogoutLoader"),
S=document.getElementById("varexLogoutSuccessIcon"),
...
g.gain.setValueAtTime(.0001,s+d);
g.gain.exponentialRampToValueAtTime(.055,s+d+.025);
g.gain.exponentialRampToValueAtTime(.0001,s+d+u);
...
a=document.getElementById("varexLogoutActions"),
p=document.getElementById("varexLogoutProgress"),
t=document.getElementById("varexLogoutTransition");
...
html{height:100%}
body{min-height:100%}
.main{margin-right:var(--sidebar-width)!important}
...
.varex-sidebar-bottom-space{height:90px}
...
.varex-logout-actions{flex-direction:column}
}
`;
...
clearTimeout(t);
t=setTimeout(varexSaveSidebarScroll,25)
},{passive:true});

s.querySelectorAll("a[href]").forEach(l=>{
l.addEventListener("pointerdown",varexSaveSidebarScroll,{passive:true});
l.addEventListener("touchstart",varexSaveSidebarScroll,{passive:true});
l.addEventListener("click",varexSaveSidebarScroll)
});
...
const old=localStorage.getItem("varex_theme");
...
const i=document.getElementById("varexThemeIcon");
const t=document.getElementById("varexThemeText");
