...
localStorage.setItem(this.keys.pendingVerification,JSON.stringify(x));return x
},
...
try{await this.authFetch("/auth/v1/resend",{method:"POST",body:JSON.stringify({type:"signup",email:this.normalizeEmail(email)})});return{success:true,message:"تم إرسال رمز تحقق جديد."}}
catch(e){return{success:false,message:this.mapAuthError(e)}}
},
...
this.storeSession(d,true);this.authorizeDevice(d.user);return true
}catch(e){
...
redirectLoggedUser(){
if((this.isLoginPage()||this.isRegisterPage())&&(this.isLoggedIn()||this.isDeviceAuthorized())){location.replace("./index.html");return true}
return false
},

async requestPasswordReset(email){
try{await this.authFetch("/auth/v1/recover",{method:"POST",body:JSON.stringify({email:this.normalizeEmail(email)})});return{success:true,message:"تم إرسال تعليمات استعادة كلمة المرور."}}
catch(e){return{success:false,message:this.mapAuthError(e)}}
},
...
try{const s=this.getSession();if(!s)return{success:false,message:"يجب تسجيل الدخول بحساب المالك أولاً."};const u=await this.authFetch("/auth/v1/user",{method:"PUT",body:JSON.stringify({password:String(n)})});s.user=u;this.storeSession(s,true);return{success:true,message:"تم تغيير كلمة المرور بنجاح."}}
catch(e){return{success:false,message:this.mapAuthError(e)}}
},
...
this.saveStaffUsers(a);return true
},
...
const type=String(o.billingType||o.type||"monthly"),now=new Date();let expiresAt="",lifetime=false;
...
requireSubscription(){if(!this.isSubscriptionGateEnabled())return true;if(this.isLoginPage()||this.isRegisterPage()||this.isVerifyEmailPage()||this.isSubscriptionPage()||this.isSubscriptionSuccessPage())return true;if(!this.isSubscriptionActive()){location.replace("./subscription.html");return false}return true},
...
for(const x of items){const i=p.findIndex(y=>String(y.id)===String(x.productId||x.id)),q=this.positiveNumber(x.quantity||x.qty||1);if(i>=0)p[i].quantity=Math.max(0,this.toNumber(p[i].quantity)-q)}
...
saveSettings(s={}){const d={...this.getSettings(),...s,updatedAt:this.now()};this.saveObject(this.keys.settings,d);localStorage.setItem("varexSettings",JSON.stringify(d));return true},
...
o.id="varexStaffOverlay";o.className="varex-staff-overlay";
...
const o=document.getElementById("varexStaffOverlay");
...
const r=await VAREX.verifyOwnerPassword(pass);
btn.disabled=false;btn.textContent="دخول إلى VAREX";
...
if(!u){btn.disabled=false;btn.textContent="دخول إلى VAREX";return}
const r=await VAREX.verifyStaffPassword(u,pass);
btn.disabled=false;btn.textContent="دخول إلى VAREX";
if(!r.success){err.textContent=r.message;return}
u.lastLogin=new Date().toLocaleString("ar-AE");
const a=VAREX.getStaffUsers(),i=a.findIndex(x=>String(x.id)===String(u.id));
if(i>=0){a[i].lastLogin=u.lastLogin;VAREX.saveStaffUsers(a)}
...
session.role=fresh.role;session.name=fresh.name;session.username=fresh.username||"";
...
/* Save temporary staff passwords from users.html */
...
const b=e.target?.closest?.("button"),oc=String(b?.getAttribute?.("onclick")||"");
...
if(pass&&email)setTimeout(()=>VAREX.setStaffPassword(email,pass),200)
},true)
}

/* ========================================================= CONFIRM ========================================================= */
let varexConfirmResolve=null,varexConfirmBypass=false;
function varexInstallConfirmUI(){
if(document.getElementById("varexConfirmOverlay"))return;
const o=document.createElement("div");o.id="varexConfirmOverlay";o.className="varex-confirm-overlay";
o.innerHTML=`<div class="varex-confirm-card"><div class="varex-confirm-brand">VAREX</div><div class="varex-confirm-icon">!</div><h2 id="varexConfirmTitle">تأكيد العملية</h2><p id="varexConfirmMessage">هل تريد متابعة العملية؟</p><div class="varex-confirm-actions"><button id="varexConfirmYes">تأكيد</button><button id="varexConfirmNo">إلغاء</button></div></div>`;
document.body.appendChild(o);
document.getElementById("varexConfirmYes").onclick=()=>varexCloseConfirm(true);
document.getElementById("varexConfirmNo").onclick=()=>varexCloseConfirm(false)
}
...
function varexCloseConfirm(r){document.getElementById("varexConfirmOverlay")?.classList.remove("show");if(varexConfirmResolve){const x=varexConfirmResolve;varexConfirmResolve=null;x(!!r)}}
...
const t=String(el?.textContent||el?.value||"").toLowerCase(),c=String(el?.className||"").toLowerCase(),oc=String(el?.getAttribute?.("onclick")||"").toLowerCase();
return oc.includes("confirm(")||["حذف","مسح","إلغاء","الغاء","استعادة","إعادة","delete","remove","cancel","reset"].some(w=>t.includes(w)||c.includes(w))
}
...
if(window.__varexConfirmInterceptorInstalled)return;window.__varexConfirmInterceptorInstalled=true;
document.addEventListener("click",async e=>{
if(varexConfirmBypass)return;
const t=e.target?.closest?.("button,a,[role='button'],input[type='button'],input[type='submit']");
if(!t||t.closest("#varexConfirmOverlay,#varexLogoutOverlay,#varexStaffOverlay")||!varexLooksLikeConfirmAction(t))return;
e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
if(!await varexConfirm(varexGuessConfirmMessage(t)))return;
varexConfirmBypass=true;const old=window.confirm;window.confirm=()=>true;
try{t.click()}finally{setTimeout(()=>{window.confirm=old;varexConfirmBypass=false},0)}
},true)
}
...
(root.querySelectorAll?.(sel)||[]).forEach(el=>{if(el.matches("input,textarea,select,option")||el.children.length)return;const m=String(el.textContent||"").match(/-?\d[\d,\s]*(?:\.\d+)?/);if(!m)return;const n=VAREX.parseNumber(m[0],NaN);if(Number.isFinite(n))el.textContent=el.textContent.replace(m[0],VAREX.formatNumber(n,2))})
}
...
const o=document.createElement("div");o.id="varexLogoutOverlay";o.className="varex-logout-overlay";
o.innerHTML=`<div class="varex-logout-card"><div class="varex-logout-brand">VAREX</div><h2>تسجيل خروج المنشأة</h2><p>سيتم إلغاء ربط هذا الجهاز بحساب المنشأة وسيحتاج المالك إلى تسجيل الدخول بالإيميل مرة أخرى.</p><div class="varex-logout-actions"><button id="varexLogoutConfirm">تأكيد تسجيل الخروج</button><button id="varexLogoutCancel">البقاء في النظام</button></div></div>`;
document.body.appendChild(o);
document.getElementById("varexLogoutCancel").onclick=()=>o.classList.remove("show");
document.getElementById("varexLogoutConfirm").onclick=()=>VAREX.logout(true)
}
...
el.classList.add("varex-user-switch-chip");
el.title="تبديل المستخدم";
el.onclick=e=>{e.preventDefault();varexSwitchUser()}
}
});
...
const r=document.getElementById("sidebarUserRole");if(r)r.textContent=VAREX.roleName(u.role)
}
...
if(e.key==="varexTypography")varexApplyGlobalTypography();
if(e.key==="varexThemeMode")varexApplyTheme(varexGetTheme());
if(e.key===VAREX.keys.staffUsers){
const s=VAREX.getStaffSession();
if(s&&!s.isOwner){
const u=VAREX.getStaffUsers().find(x=>String(x.id)===String(s.id));
if(!u||u.status==="disabled")varexSwitchUser()
}
}
});
