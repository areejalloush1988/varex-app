...
try{
return JSON.parse(
localStorage.getItem(this.keys.cachedUser)||"null"
)
}catch(e){
return null
}
...
redirectLoggedUser(){
if(
(this.isLoginPage()||this.isRegisterPage())&&
this.isLoggedIn()
){
...
try{
const x=JSON.parse(
localStorage.getItem(this.keys.subscription)||"null"
);
...
try{
const x=JSON.parse(
localStorage.getItem("varexSettings")||
"null"
);
...
/*
   أثناء التطوير يبقى قفل الاشتراك غير مفعل.
   عند جاهزية الدفع والترخيص نفعله.
*/
...
/*
   صفحة نجاح الاشتراك تعتبر جزءاً من
   تبويب الاشتراك والترخيص حتى يبقى
   التبويب نفسه Active.
*/
...
/*
   إذا كان قفل الاشتراك فعالاً، نفحص الحالة.
   subscription.html و subscription-success.html
   مسموح بفتحهما من requireSubscription().
*/
...
/*
   مهم:
   لم نعد نوقف بناء القائمة في صفحة الاشتراك.
   الآن صفحة subscription.html وصفحة النجاح
   تستخدمان نفس القائمة الجانبية مثل باقي VAREX.
*/
