...
async callPaymentService(action,data={}){let s=this.getSession();if(!s?.access_token)throw new Error("يلزم تسجيل الدخول بحساب مالك المنشأة.");if((s.expires_at||0)<=Math.floor(Date.now()/1000)+60){const ok=await this.refreshSession();if(!ok)throw new Error("انتهت جلسة المستخدم. يرجى تسجيل الدخول من جديد.");s=this.getSession()}const r=await fetch(this.config.supabaseUrl+"/functions/v1/paypal-checkout",{method:"POST",headers:{apikey:this.config.supabaseKey,Authorization:`Bearer ${s.access_token}`,"Content-Type":"application/json"},body:JSON.stringify({action,...data})});const x=await r.json().catch(()=>({}));if(!r.ok||x?.success===false)throw new Error(x?.message||"تعذر الاتصال بخدمة الدفع الآمن.");return x},
