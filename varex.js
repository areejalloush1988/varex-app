...
sessionStorage.removeItem(this.keys.session);localStorage.removeItem(this.keys.session);localStorage.removeItem(this.keys.cachedUser);sessionStorage.removeItem("varex_authenticated");localStorage.removeItem("varex_authenticated");
...
if(!mail)return{success:false,message:"البريد الإلكتروني غير موجود."};
try{await this.authFetch("/auth/v1/resend",{method:"POST",body:JSON.stringify({type:"signup",email:mail})});return{success:true,message:"تم إرسال رمز تحقق جديد إلى بريدك الإلكتروني."}}catch(e){return{success:false,message:this.mapAuthError(e)}}
},
...
try{const d=await this.authFetch("/auth/v1/token?grant_type=refresh_token",{method:"POST",body:JSON.stringify({refresh_token:s.refresh_token})});this.storeSession(d,s.remember);return true}catch(e){this.logout(false);return false}
},
...
try{await this.authFetch("/auth/v1/recover",{method:"POST",body:JSON.stringify({email:mail})});return{success:true,message:"تم إرسال تعليمات استعادة كلمة المرور إلى بريدك الإلكتروني."}}catch(e){return{success:false,message:this.mapAuthError(e)}}
},
...
/* =========================================================
   UNIFIED VAREX MENU + FIXED SCROLL POSITION
========================================================= */
...
if(!nav)return;
try{sessionStorage.setItem(VAREX_SIDEBAR_SCROLL_KEY,String(nav.scrollTop))}catch(e){}
}
...
if(!nav)return;
let saved=0;
try{saved=Number(sessionStorage.getItem(VAREX_SIDEBAR_SCROLL_KEY)||0)}catch(e){}
if(!Number.isFinite(saved)||saved<0)saved=0;

requestAnimationFrame(()=>{
requestAnimationFrame(()=>{
const max=Math.max(0,nav.scrollHeight-nav.clientHeight);
nav.scrollTop=Math.min(saved,max);
});
});
}
...
return`<a href="./${file}" class="${active.trim()}"><span class="nav-icon">${icon}</span><span>${title}</span></a>`;
}).join("");

varexAddSidebarActions();

nav.addEventListener("scroll",varexSaveSidebarScroll,{passive:true});
...
style.textContent=`
.varex-sidebar-actions{margin-top:16px;padding-top:14px;border-top:1px solid rgba(255,255,255,.12)}
.varex-sidebar-actions button{width:100%;height:48px;min-height:48px;display:flex;align-items:center;justify-content:flex-start;gap:11px;padding:0 14px;margin:0 0 5px;border:0;border-radius:10px;font-family:inherit;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;transition:.2s}
.varex-theme-button{background:rgba(255,255,255,.08);color:#dbeafe}
.varex-theme-button:hover{background:rgba(255,255,255,.14);color:#fff}
.varex-logout-button{background:rgba(220,38,38,.14);color:#fecaca}
.varex-logout-button:hover{background:#dc2626;color:#fff}
.varex-power-icon{width:25px;height:25px;min-width:25px;border:1.5px solid currentColor;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;line-height:1}
`;
...
body.varex-dark .panel,body.varex-dark .card,body.varex-dark .stat,body.varex-dark .stat-card,body.varex-dark .summary-card,body.varex-dark .dashboard-card,body.varex-dark .widget,body.varex-dark .box,body.varex-dark .section-card,body.varex-dark .table-card,body.varex-dark .modal,body.varex-dark .modal-content{background:#132641!important;border-color:#29415f!important;color:#f8fafc!important;box-shadow:0 5px 20px rgba(0,0,0,.18)!important}
body.varex-dark h1,body.varex-dark h2,body.varex-dark h3,body.varex-dark h4,body.varex-dark h5,body.varex-dark h6,body.varex-dark .hero h1,body.varex-dark .page-name h2,body.varex-dark .panel h2,body.varex-dark .card h2,body.varex-dark .card h3,body.varex-dark .stat strong{color:#fff!important}
body.varex-dark p,body.varex-dark small,body.varex-dark label,body.varex-dark .muted,body.varex-dark .hero p,body.varex-dark .page-name small,body.varex-dark .stat span{color:#cbd5e1!important}
body.varex-dark input,body.varex-dark select,body.varex-dark textarea{background:#0e2039!important;border-color:#35506f!important;color:#fff!important}
body.varex-dark input::placeholder,body.varex-dark textarea::placeholder{color:#8293aa!important}
body.varex-dark input:focus,body.varex-dark select:focus,body.varex-dark textarea:focus{border-color:#6b8bb1!important;box-shadow:0 0 0 3px rgba(96,165,250,.10)!important}
...
body.varex-dark .chip,body.varex-dark .info-chip{background:#162b48!important;border-color:#304967!important;color:#cbd5e1!important}
body.varex-dark .chip strong,body.varex-dark .info-chip strong{color:#fff!important}
body.varex-dark .page-icon{background:#1b3150!important;color:#fff!important}
body.varex-dark .note,body.varex-dark .system-info,body.varex-dark .security-note,body.varex-dark .security-status,body.varex-dark .summary{background:#10233d!important;border-color:#2b4463!important;color:#dbeafe!important}
...
body.varex-dark .secondary,body.varex-dark .mini,body.varex-dark .reset{background:#1a3150!important;border-color:#35506f!important;color:#f8fafc!important}
body.varex-dark .secondary:hover,body.varex-dark .mini:hover,body.varex-dark .reset:hover{background:#203b5e!important}
body.varex-dark .primary,body.varex-dark .save{background:linear-gradient(135deg,#244a78,#18345a)!important;color:#fff!important}
body.varex-dark .modal-bg{background:rgba(2,8,23,.76)!important}
body.varex-dark .empty{color:#94a3b8!important}
body.varex-dark .sidebar{background:linear-gradient(180deg,#172554 0%,#13234f 48%,#0f1d43 100%)!important;color:#fff!important}
...
body.varex-dark .sidebar .system-status{color:#d1fae5!important}
body.varex-dark .sidebar .nav a{color:#dbeafe!important}
body.varex-dark .sidebar .nav a:hover{background:rgba(255,255,255,.08)!important;color:#fff!important}
body.varex-dark .sidebar .nav a.active{background:#fff!important;color:#172554!important;box-shadow:0 5px 15px rgba(0,0,0,.18)!important}
...
body.varex-dark .varex-theme-button{background:rgba(255,255,255,.08)!important;color:#dbeafe!important}
body.varex-dark .varex-theme-button:hover{background:rgba(255,255,255,.14)!important;color:#fff!important}
body.varex-dark .varex-logout-button{background:rgba(220,38,38,.14)!important;color:#fecaca!important}
body.varex-dark .varex-logout-button:hover{background:#dc2626!important;color:#fff!important}
...
/* حفظ آخر مكان حركتي إليه القائمة */
