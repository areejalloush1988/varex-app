...
/* BUSINESS ID */
clearBusinessIdCache(){try{const s=this.getSession(),uid=s?.user?.id;if(uid)localStorage.removeItem(`${this.keys.businessId}__${uid}`);localStorage.removeItem(this.keys.businessId)}catch(e){}},async getCurrentBusinessId(force=false){let s=this.getSession();if(!s?.access_token||!s?.user?.id)throw new Error("لا توجد جلسة مستخدم صالحة.");if((s.expires_at||0)<=Math.floor(Date.now()/1000)+60){const ok=await this.refreshSession();if(!ok)throw new Error("انتهت جلسة المستخدم. يرجى تسجيل الدخول من جديد.");s=this.getSession()}const cacheKey=`${this.keys.businessId}__${s.user.id}`;if(!force){const cached=localStorage.getItem(cacheKey);if(cached)return cached}const result=await this.dbFetch("rpc/get_current_business_id",{method:"POST",body:"{}"});let businessId=null;if(typeof result==="string")businessId=result;else if(Array.isArray(result))businessId=result[0]?.get_current_business_id||result[0]?.business_id||result[0]||null;else if(result&&typeof result==="object")businessId=result.get_current_business_id||result.business_id||result.id||null;if(!businessId)throw new Error("لم يتم العثور على منشأة مرتبطة بهذا الحساب.");businessId=String(businessId).replace(/^"|"$/g,"").trim();if(!businessId)throw new Error("لم يتم العثور على منشأة مرتبطة بهذا الحساب.");localStorage.setItem(cacheKey,businessId);return businessId},

/* SUPABASE PRODUCTS */
mapDbProduct(row,suppliers={}){if(!row)return null;return{id:row.id,createdAt:row.created_at||"",updatedAt:row.updated_at||"",businessId:row.business_id||"",name:row.name||"",sku:row.sku||"",barcode:row.barcode||"",category:row.category||"",cost:this.toNumber(row.cost),price:this.toNumber(row.price),quantity:this.toNumber(row.quantity),minimumStock:this.toNumber(row.minimum_stock),unit:row.unit||"piece",status:row.status||"active",description:row.description||"",supplierId:row.supplier_id||"",supplier:row.supplier_id?(suppliers[row.supplier_id]||""):"",taxRate:this.toNumber(row.tax_rate),imageUrl:row.image_url||""}},async getSupplierMap(){try{const rows=await this.dbFetch("suppliers?select=id,name&order=name.asc");const map={};(Array.isArray(rows)?rows:[]).forEach(x=>{if(x?.id)map[x.id]=x.name||""});return map}catch(e){console.warn("VAREX SUPPLIERS MAP:",e);return{}}},async resolveSupplierIdByName(name){const n=this.cleanText(name);if(!n)return null;const rows=await this.dbFetch("suppliers?select=id,name&name=eq."+encodeURIComponent(n)+"&limit=1");return Array.isArray(rows)&&rows[0]?.id?rows[0].id:null},async syncProductsFromSupabase(){const[rows,supplierMap]=await Promise.all([this.dbFetch("products?select=*&order=created_at.asc"),this.getSupplierMap()]);const products=(Array.isArray(rows)?rows:[]).map(x=>this.mapDbProduct(x,supplierMap)).filter(Boolean);this.saveProducts(products);return products},async addProductRemote(p={}){const businessId=await this.getCurrentBusinessId();let supplierId=p.supplierId||null;if(!supplierId&&this.cleanText(p.supplier)){supplierId=await this.resolveSupplierIdByName(p.supplier);if(!supplierId)throw new Error("المورد المكتوب غير موجود في قائمة الموردين. أضف المورد أولاً أو اترك خانة المورد فارغة.")}const body={business_id:businessId,name:this.cleanText(p.name||p.productName),sku:this.cleanText(p.sku||"")||null,barcode:this.cleanText(p.barcode)||null,category:this.cleanText(p.category)||null,cost:this.positiveNumber(p.cost||p.costPrice),price:this.positiveNumber(p.price||p.salePrice),quantity:this.positiveNumber(p.quantity),minimum_stock:this.positiveNumber(p.minimumStock),unit:this.cleanText(p.unit)||"piece",status:this.cleanText(p.status)||"active",description:this.cleanText(p.description)||null,supplier_id:supplierId,tax_rate:this.positiveNumber(p.taxRate),image_url:this.cleanText(p.imageUrl)||null,updated_at:this.now()};if(!body.name)throw new Error("اسم المنتج مطلوب.");const rows=await this.dbFetch("products?select=*",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify(body)});await this.syncProductsFromSupabase();return Array.isArray(rows)&&rows[0]?this.mapDbProduct(rows[0],await this.getSupplierMap()):null},async updateProductRemote(id,c={}){if(!id)throw new Error("معرّف المنتج غير موجود.");const body={updated_at:this.now()};if(c.name!==undefined)body.name=this.cleanText(c.name);if(c.sku!==undefined)body.sku=this.cleanText(c.sku)||null;if(c.barcode!==undefined)body.barcode=this.cleanText(c.barcode)||null;if(c.category!==undefined)body.category=this.cleanText(c.category)||null;if(c.cost!==undefined)body.cost=this.positiveNumber(c.cost);if(c.price!==undefined)body.price=this.positiveNumber(c.price);if(c.quantity!==undefined)body.quantity=this.positiveNumber(c.quantity);if(c.minimumStock!==undefined)body.minimum_stock=this.positiveNumber(c.minimumStock);if(c.unit!==undefined)body.unit=this.cleanText(c.unit)||"piece";if(c.status!==undefined)body.status=this.cleanText(c.status)||"active";if(c.description!==undefined)body.description=this.cleanText(c.description)||null;if(c.taxRate!==undefined)body.tax_rate=this.positiveNumber(c.taxRate);if(c.imageUrl!==undefined)body.image_url=this.cleanText(c.imageUrl)||null;if(c.supplier!==undefined){const n=this.cleanText(c.supplier);body.supplier_id=n?await this.resolveSupplierIdByName(n):null;if(n&&!body.supplier_id)throw new Error("المورد المكتوب غير موجود في قائمة الموردين.")}await this.dbFetch("products?id=eq."+encodeURIComponent(id),{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(body)});await this.syncProductsFromSupabase();return this.getProductById(id)},async deleteProductRemote(id){if(!id)return false;await this.dbFetch("products?id=eq."+encodeURIComponent(id),{method:"DELETE",headers:{Prefer:"return=minimal"}});await this.syncProductsFromSupabase();return true},async adjustStockRemote(id,change){const product=this.getProductById(id)||((await this.syncProductsFromSupabase()).find(x=>String(x.id)===String(id)));if(!product)throw new Error("المنتج غير موجود.");const next=this.toNumber(product.quantity)+this.toNumber(change);if(next<0)throw new Error("الكمية المتوفرة غير كافية.");await this.dbFetch("products?id=eq."+encodeURIComponent(id),{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({quantity:next,updated_at:this.now()})});await this.syncProductsFromSupabase();return this.getProductById(id)},

/* BUSINESS DATA */
getProducts(){return this.getData(this.keys.products)},saveProducts(x){return this.saveData(this.keys.products,x)},getProductById(id){return this.getProducts().find(x=>String(x.id)===String(id))||null},findProductByBarcode(b){return this.getProducts().find(x=>this.cleanText(x.barcode)===this.cleanText(b))||null},addProduct(p={}){const a=this.getProducts(),x={...p,id:p.id||this.generateId("PRD"),name:this.cleanText(p.name||p.productName),quantity:this.positiveNumber(p.quantity),price:this.positiveNumber(p.price||p.salePrice),cost:this.positiveNumber(p.cost||p.costPrice),createdAt:p.createdAt||this.now(),updatedAt:this.now()};a.push(x);this.saveProducts(a);return x},updateProduct(id,c={}){const a=this.getProducts(),i=a.findIndex(x=>String(x.id)===String(id));if(i<0)return false;a[i]={...a[i],...c,id:a[i].id,updatedAt:this.now()};this.saveProducts(a);return a[i]},deleteProduct(id){const a=this.getProducts(),b=a.filter(x=>String(x.id)!==String(id));this.saveProducts(b);return b.length!==a.length},adjustStock(id,n){const a=this.getProducts(),i=a.findIndex(x=>String(x.id)===String(id));if(i<0)return false;a[i].quantity=Math.max(0,this.toNumber(a[i].quantity)+this.toNumber(n));this.saveProducts(a);return a[i]},getSales(){return this.getData(this.keys.sales)},saveSales(x){return this.saveData(this.keys.sales,x)},getSaleById(id){return this.getSales().find(x=>String(x.id)===String(id))||null},addSale(s={}){const a=this.getSales(),x={...s,id:s.id||this.generateId("SAL"),invoiceNumber:s.invoiceNumber||`INV-${Date.now()}`,createdAt:s.createdAt||this.now(),date:s.date||this.now(),updatedAt:this.now()};a.push(x);this.saveSales(a);return x},deleteSale(id){const a=this.getSales(),b=a.filter(x=>String(x.id)!==String(id));this.saveSales(b);return b.length!==a.length},completeSale(s={}){const items=Array.isArray(s.items)?s.items:[];if(!items.length)return{success:false,message:"لا توجد منتجات في الفاتورة."};const p=this.getProducts();for(const x of items){const i=p.findIndex(y=>String(y.id)===String(x.productId||x.id)),q=this.positiveNumber(x.quantity||x.qty||1);if(i>=0&&q>this.toNumber(p[i].quantity))return{success:false,message:"الكمية غير متوفرة للمنتج: "+(p[i].name||"")}}for(const x of items){const i=p.findIndex(y=>String(y.id)===String(x.productId||x.id)),q=this.positiveNumber(x.quantity||x.qty||1);if(i>=0)p[i].quantity=Math.max(0,this.toNumber(p[i].quantity)-q)}this.saveProducts(p);return{success:true,sale:this.addSale(s)}},getCustomers(){return this.getData(this.keys.customers)},saveCustomers(x){return this.saveData(this.keys.customers,x)},addCustomer(x={}){return this._add(this.keys.customers,"CUS",x)},updateCustomer(id,c={}){return this._update(this.keys.customers,id,c)},deleteCustomer(id){return this._delete(this.keys.customers,id)},getSuppliers(){return this.getData(this.keys.suppliers)},saveSuppliers(x){return this.saveData(this.keys.suppliers,x)},addSupplier(x={}){return this._add(this.keys.suppliers,"SUP",x)},updateSupplier(id,c={}){return this._update(this.keys.suppliers,id,c)},deleteSupplier(id){return this._delete(this.keys.suppliers,id)},getEmployees(){return this.getData(this.keys.employees)},saveEmployees(x){return this.saveData(this.keys.employees,x)},addEmployee(x={}){return this._add(this.keys.employees,"EMP",x)},updateEmployee(id,c={}){return this._update(this.keys.employees,id,c)},deleteEmployee(id){return this._delete(this.keys.employees,id)},getTransactions(){return this.getData(this.keys.transactions)},saveTransactions(x){return this.saveData(this.keys.transactions,x)},addTransaction(x={}){return this._add(this.keys.transactions,"TRX",{...x,amount:this.positiveNumber(x.amount),date:x.date||this.today()})},updateTransaction(id,c={}){return this._update(this.keys.transactions,id,c)},deleteTransaction(id){return this._delete(this.keys.transactions,id)},getHeldSales(){return this.getData(this.keys.heldSales)},saveHeldSales(x){return this.saveData(this.keys.heldSales,x)},holdSale(x={}){return this._add(this.keys.heldSales,"HOLD",x)},removeHeldSale(id){return this._delete(this.keys.heldSales,id)},_add(k,p,x={}){const a=this.getData(k),o={...x,id:x.id||this.generateId(p),createdAt:x.createdAt||this.now(),updatedAt:this.now()};a.push(o);this.saveData(k,a);return o},_update(k,id,c={}){const a=this.getData(k),i=a.findIndex(x=>String(x.id)===String(id));if(i<0)return false;a[i]={...a[i],...c,id:a[i].id,updatedAt:this.now()};this.saveData(k,a);return a[i]},_delete(k,id){const a=this.getData(k),b=a.filter(x=>String(x.id)!==String(id));this.saveData(k,b);return b.length!==a.length},getSettings(){const d={businessName:"VAREX",currency:"AED",currencySymbol:"د.إ",taxEnabled:true,taxRate:5,lowStockLimit:5,language:"ar"},p=this.getObject(this.keys.settings,{});return{...d,...p}},saveSettings(s={}){const d={...this.getSettings(),...s,updatedAt:this.now()};this.saveObject(this.keys.settings,d);return true},money(v){const s=this.getSettings(),sym=this.cleanText(s.currencySymbol)||(s.currency==="AED"?"د.إ":s.currency);return`${this.formatNumber(v,2)} ${sym}`},calculateTax(v){const s=this.getSettings();return s.taxEnabled===false?0:this.positiveNumber(v)*this.toNumber(s.taxRate,5)/100},getTodaySales(){const t=this.today();return this.getSales().filter(s=>this.normalizeDate(s.createdAt||s.date||s.saleDate||s.invoiceDate||"")===t)},getTodaySalesTotal(){return this.getTodaySales().reduce((a,s)=>a+this.toNumber(s.total??s.grandTotal??s.finalTotal??s.netTotal??s.amount??0),0)},getStockAlerts(){const l=this.toNumber(this.getSettings().lowStockLimit,5);return this.getProducts().filter(p=>this.toNumber(p.quantity)<=this.toNumber(p.minimumStock,l))},

/* INTERNAL USERS */
getStaffUsers(){try{const x=JSON.parse(localStorage.getItem(this.scopeKey(this.keys.staffUsers))||"[]");return Array.isArray(x)?x:[]}catch(e){return[]}},saveStaffUsers(x){localStorage.setItem(this.scopeKey(this.keys.staffUsers),JSON.stringify(Array.isArray(x)?x:[]));return true},getActiveStaffUsers(){return this.getStaffUsers().filter(x=>x.status!=="disabled")},getStaffSession(){try{const x=JSON.parse(sessionStorage.getItem(this.keys.staffSession)||"null");return x&&typeof x==="object"?x:null}catch(e){return null}},setStaffSession(u){const x={id:u.id,name:u.name||"المستخدم",email:u.email||"",username:u.username||"",role:u.role||"custom",permissions:Array.isArray(u.permissions)?u.permissions:[],isOwner:!!u.isOwner,loginAt:this.now()};sessionStorage.setItem(this.keys.staffSession,JSON.stringify(x));return x},clearStaffSession(){sessionStorage.removeItem(this.keys.staffSession)},getActiveOperator(){return this.getStaffSession()},roleName(r){return{owner:"المالك",manager:"المدير",accountant:"المحاسب",cashier:"الكاشير",custom:"مستخدم"}[r]||"مستخدم"},getOwnerOperator(){const d=this.getDeviceOwner(),u=this.getCurrentUser()||{};return{id:"owner",name:d?.name||u.name||"مالك المنشأة",email:d?.email||u.email||"",username:d?.username||u.username||"owner",role:"owner",permissions:["*"],isOwner:true}},async hashPassword(v){const text=String(v||"");if(window.crypto?.subtle){const b=new TextEncoder().encode(text),h=await crypto.subtle.digest("SHA-256",b);return Array.from(new Uint8Array(h)).map(x=>x.toString(16).padStart(2,"0")).join("")}let h=5381;for(let i=0;i<text.length;i++)h=((h<<5)+h)^text.charCodeAt(i);return"fallback-"+(h>>>0).toString(16)},async setStaffPassword(email,password){const a=this.getStaffUsers(),mail=this.normalizeEmail(email),i=a.findIndex(x=>this.normalizeEmail(x.email)===mail);if(i<0)return false;a[i].passwordHash=await this.hashPassword(password);a[i].passwordUpdatedAt=this.now();if(!a[i].username)a[i].username=this.normalizeUsername((a[i].name||mail.split("@")[0]).replace(/\s+/g,"."));this.saveStaffUsers(a);return true},async verifyStaffPassword(u,password){if(!u?.passwordHash)return{success:false,message:"لم يتم تعيين كلمة مرور لهذا المستخدم. عدّل المستخدم وحدد كلمة مرور ثم احفظ."};const h=await this.hashPassword(password);return h===u.passwordHash?{success:true}:{success:false,message:"كلمة المرور غير صحيحة."}},hasPermission(p){const u=this.getStaffSession();if(!u)return false;if(u.isOwner||u.permissions?.includes("*"))return true;return Array.isArray(u.permissions)&&u.permissions.includes(p)},
...
/* ========================================================= ANTI SHAKE BOOT ========================================================= */
(function(){
const publicPage=/\/(login|register|verify-email|reset-password)\.html$/i.test(location.pathname);
if(publicPage)return;
const mode=localStorage.getItem("varexThemeMode");
const oldTheme=localStorage.getItem("varex_theme");
const dark=mode==="dark"||(mode==="system"&&window.matchMedia?.("(prefers-color-scheme:dark)").matches)||(mode!=="light"&&mode!=="dark"&&mode!=="system"&&oldTheme==="dark");
document.documentElement.style.background=dark?"#081426":"#f4f6fb";
document.documentElement.classList.remove("varex-ui-ready");
const s=document.createElement("style");
s.id="varexAntiShakeBoot";
s.textContent=`
html{
overflow-y:scroll!important;
overflow-x:hidden!important;
scrollbar-gutter:stable!important;
background:${dark?"#081426":"#f4f6fb"}!important;
width:100%!important;
...
html.varex-ui-ready body{
visibility:visible!important;
opacity:1!important;
transition:none!important
}
html.varex-page-leaving body{
...
.content,main.content,.page-content,.main-content{
min-width:0!important;
transform:none!important;
translate:none!important
}
.hero,.panel,.stat,.stat-card,.status-card,.plan,.card,.dashboard-card,.summary-card,.metric-card,.kpi-card,.widget{
translate:none!important
}
`;
...
async function varexLoginSelectedStaff(){const pass=document.getElementById("varexStaffPassword").value,err=document.getElementById("varexStaffError"),btn=document.getElementById("varexStaffLoginBtn");if(!pass){err.textContent="أدخل كلمة المرور.";return}if(btn.disabled)return;err.textContent="";btn.disabled=true;btn.innerHTML=`<span class="varex-mini-spinner"></span><span>جاري تسجيل الدخول...</span>`;await varexWait(550);let result;if(varexSelectedStaffId==="__owner__"){result=await VAREX.verifyOwnerPassword(pass);if(result.success)VAREX.setStaffSession(VAREX.getOwnerOperator())}else{const u=VAREX.getStaffUsers().find(x=>String(x.id)===String(varexSelectedStaffId));if(!u){btn.disabled=false;btn.textContent="دخول إلى VAREX";return}result=await VAREX.verifyStaffPassword(u,pass);if(result.success)VAREX.setStaffSession(u)}if(!result?.success){btn.disabled=false;btn.textContent="دخول إلى VAREX";err.textContent=result?.message||"تعذر تسجيل الدخول.";return}btn.innerHTML="✓ تم تسجيل الدخول بنجاح";await varexWait(650);varexCloseStaffGate();btn.disabled=false;btn.textContent="دخول إلى VAREX";varexBuildMenu(true);varexInstallTopSwitchUserButton();varexShowCurrentUser();varexCheckCurrentPermission()}
...
const signature=(operator?.id||"none")+"|"+current+"|"+(operator?.permissions||[]).join(",");
if(!force&&nav.dataset.varexSignature===signature&&nav.querySelector(".varex-sidebar-actions"))return;
nav.dataset.varexSignature=signature;
...
function varexInstallStableNavigation(){
document.querySelectorAll(".sidebar .nav a[href]").forEach(a=>{
if(a.dataset.varexStableNav==="1")return;
a.dataset.varexStableNav="1";
a.addEventListener("click",e=>{
if(e.ctrlKey||e.metaKey||e.shiftKey||e.altKey||e.button!==0)return;
const href=a.getAttribute("href");
if(!href||href.startsWith("#")||a.classList.contains("active"))return;
document.documentElement.classList.add("varex-page-leaving")
})
})
}
...
btn.innerHTML="👥 <strong>تبديل المستخدم</strong>";
...
function varexApplyTheme(t){document.documentElement.setAttribute("data-theme",t);document.documentElement.setAttribute("data-varex-theme",t);document.documentElement.style.background=t==="dark"?"#081426":"#f4f6fb";document.body?.classList.toggle("varex-dark",t==="dark");localStorage.setItem("varex_theme",t);varexUpdateThemeButton()}
...
["sidebarUserName","topUserName","settingsUserName","currentUserName","currentUser"].forEach(id=>{
const e=document.getElementById(id);
if(e)e.textContent=u.name||u.username||"المستخدم"
})
}
...
.nav-label{white-space:nowrap!important}
...
.hero,.panel,.stat,.stat-card,.status-card,.plan,.card,.dashboard-card,.dashboard-cards,.summary-card,.metric-card,.kpi-card,.widget,.top-info,.info-chip,.chip{
translate:none!important
}
...
body.varex-dark .varex-selected-user{color:#fff}
...
/* ========================================================= REVEAL + START ========================================================= */
let varexUiRevealed=false;

function varexRevealUI(){
if(varexUiRevealed)return;
varexUiRevealed=true;
requestAnimationFrame(()=>{
requestAnimationFrame(()=>{
setTimeout(()=>{
document.documentElement.classList.remove("varex-page-leaving");
document.documentElement.classList.add("varex-ui-ready")
},20)
})
})
}
