...
if(!r.ok){const err=new Error(data.msg||data.message||data.error_description||data.error||"تعذر الاتصال بخدمة الحسابات.");err.status=r.status;err.data=data;throw err}
return data
},
...
try{localStorage.setItem(this.keys.subscription,JSON.stringify(x));return x}catch(e){console.error(e);return false}
},

generateLicenseKey(){
const chars="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
let result="VAREX";
for(let g=0;g<4;g++){result+="-";for(let i=0;i<4;i++)result+=chars[Math.floor(Math.random()*chars.length)]}
return result
},
...
if(expiry.getTime()<=Date.now()){this.expireSubscription();return false}
return true
},
...
const restore=()=>{const max=Math.max(0,sidebar.scrollHeight-sidebar.clientHeight);sidebar.scrollTop=Math.min(saved,max)};
requestAnimationFrame(()=>requestAnimationFrame(()=>{restore();setTimeout(restore,40);setTimeout(restore,120)}))
}
...
const[file,icon,title]=item,active=current===file.toLowerCase()?" active":"";
...
width:var(--sidebar-width)!important;height:100vh!important;height:100dvh!important;
...
-webkit-overflow-scrolling:touch!important;overscroll-behavior-y:contain!important;overscroll-behavior-x:none!important;
touch-action:pan-y!important;scroll-behavior:auto!important;z-index:1000!important;
box-shadow:-4px 0 18px rgba(15,29,67,.10)!important
}

.sidebar .brand{
position:relative!important;top:auto!important;width:100%!important;
height:var(--brand-height,170px)!important;min-height:var(--brand-height,170px)!important;
max-height:var(--brand-height,170px)!important;flex:none!important
}
...
gap:12px!important;padding:0 15px!important;margin:0 0 8px!important;border-radius:11px!important;
font-family:inherit!important;font-size:14px!important;font-weight:600!important;line-height:1.4!important;
white-space:nowrap!important;text-decoration:none!important;background:rgba(255,255,255,.055)!important;color:#dbeafe!important;
border-top:1px solid rgba(255,255,255,.12)!important;border-left:1px solid rgba(255,255,255,.08)!important;
border-right:1px solid rgba(0,0,0,.10)!important;border-bottom:3px solid rgba(5,14,37,.44)!important;
...
.sidebar .nav a:hover{
background:rgba(255,255,255,.11)!important;color:#fff!important;transform:translateY(-1px)!important
}

.sidebar .nav a:active{
transform:translateY(2px)!important;border-bottom-width:1px!important;margin-bottom:10px!important;
background:rgba(255,255,255,.15)!important
}

.sidebar .nav a.active{
background:#fff!important;color:#172554!important;font-weight:700!important;
border-top:1px solid #fff!important;border-left:1px solid #f8fafc!important;
border-right:1px solid #cbd5e1!important;border-bottom:3px solid #94a3b8!important;
box-shadow:none!important
}

.sidebar .nav a.active:hover{background:#f8fafc!important;color:#172554!important}
.sidebar .nav a.active:active{background:#f1f5f9!important;border-bottom-width:1px!important}
.sidebar .nav a.active .nav-icon,.sidebar .nav a.active .nav-label{color:#172554!important}

.sidebar .nav .nav-label{
font-size:14px!important;font-weight:600!important;line-height:1.4!important;white-space:nowrap!important
}

.sidebar .nav .nav-icon{
width:27px!important;height:27px!important;min-width:27px!important;min-height:27px!important;
display:flex!important;align-items:center!important;justify-content:center!important;font-size:17px!important;flex-shrink:0!important
}

.sidebar-footer{
position:relative!important;width:100%!important;height:54px!important;min-height:54px!important;
display:flex!important;align-items:center!important;justify-content:center!important
}

.varex-sidebar-actions{
width:100%;margin-top:18px;padding-top:15px;border-top:1px solid rgba(255,255,255,.12)
}

.varex-sidebar-actions button{
width:100%;height:50px;min-height:50px;display:flex;align-items:center;justify-content:flex-start;
gap:12px;padding:0 15px;margin:0 0 9px;border-radius:11px;font-family:inherit;font-size:14px;
font-weight:700;cursor:pointer;white-space:nowrap;box-shadow:none!important;transform:translateY(0);
...
.varex-logout-button:hover{background:#f1f5f9!important;color:#172554!important;transform:translateY(-1px)}
.varex-logout-button:active{transform:translateY(2px);border-bottom-width:1px!important;margin-bottom:11px}

.varex-power-icon{
width:29px;height:29px;min-width:29px;min-height:29px;border:2px solid #172554;border-radius:50%;
display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;line-height:1;
flex-shrink:0;background:#f8fafc;color:#172554
}

.varex-sidebar-bottom-space{display:block;width:100%;height:90px;min-height:90px;pointer-events:none}

.sidebar{scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.23) transparent}
.sidebar::-webkit-scrollbar{width:6px}
.sidebar::-webkit-scrollbar-track{background:transparent}
.sidebar::-webkit-scrollbar-thumb{background:rgba(255,255,255,.23);border-radius:10px}
...
.sidebar{position:fixed!important;right:0!important;top:0!important;bottom:0!important;width:var(--sidebar-width)!important;height:100dvh!important;display:block!important;overflow-y:auto!important}
.sidebar .nav{display:block!important;overflow:visible!important;padding:16px 14px 0!important}
.sidebar .nav a{width:100%!important;min-width:0!important;margin:0 0 8px!important}
.main{margin-right:var(--sidebar-width)!important}
}
...
if(document.body)document.body.classList.toggle("varex-dark",theme==="dark");
...
const dark=varexGetTheme()==="dark",icon=document.getElementById("varexThemeIcon"),text=document.getElementById("varexThemeText");
...
body.varex-dark{background:#081426!important;color:#f8fafc!important}
body.varex-dark .main,body.varex-dark main,body.varex-dark .content{background:#081426!important;color:#f8fafc!important}
body.varex-dark .topbar{background:#101f38!important;border-color:#263955!important;color:#f8fafc!important}
...
background:#132641!important;border-color:#29415f!important;color:#f8fafc!important;
box-shadow:0 5px 20px rgba(0,0,0,.18)!important
}

body.varex-dark h1,body.varex-dark h2,body.varex-dark h3,body.varex-dark h4,
body.varex-dark h5,body.varex-dark h6,body.varex-dark .hero h1,
body.varex-dark .page-name h2,body.varex-dark .panel h2,
body.varex-dark .card h2,body.varex-dark .card h3,body.varex-dark .stat strong{color:#fff!important}

body.varex-dark p,body.varex-dark small,body.varex-dark label,body.varex-dark .muted,
body.varex-dark .hero p,body.varex-dark .page-name small,body.varex-dark .stat span{color:#cbd5e1!important}

body.varex-dark input,body.varex-dark select,body.varex-dark textarea{
background:#0e2039!important;border-color:#35506f!important;color:#fff!important
}

body.varex-dark input::placeholder,body.varex-dark textarea::placeholder{color:#8293aa!important}

body.varex-dark input:focus,body.varex-dark select:focus,body.varex-dark textarea:focus{
border-color:#6b8bb1!important;box-shadow:0 0 0 3px rgba(96,165,250,.10)!important
}

body.varex-dark table{background:#132641!important;color:#f8fafc!important}
body.varex-dark thead,body.varex-dark th{background:#0e2039!important;color:#dbeafe!important;border-color:#29415f!important}
...
body.varex-dark .info-chip strong{color:#172554!important}

body.varex-dark .page-icon{background:#1b3150!important;color:#fff!important}
...
background:#10233d!important;border-color:#2b4463!important;color:#dbeafe!important
}

body.varex-dark .switch-row{color:#e2e8f0!important;border-color:#29415f!important}
body.varex-dark .filters{border-color:#29415f!important}

body.varex-dark .secondary,
body.varex-dark .mini,
body.varex-dark .reset{
background:#1a3150!important;border-color:#35506f!important;color:#f8fafc!important
}

body.varex-dark .secondary:hover,
body.varex-dark .mini:hover,
body.varex-dark .reset:hover{background:#203b5e!important}

body.varex-dark .primary,
body.varex-dark .save{background:linear-gradient(135deg,#244a78,#18345a)!important;color:#fff!important}

body.varex-dark .modal,
body.varex-dark .modal-bg,
body.varex-dark .modal-overlay{background-color:rgba(2,8,23,.76)!important}
...
body.varex-dark .sidebar .brand{border-color:rgba(255,255,255,.10)!important}
...
body.varex-dark .sidebar .nav a:hover{background:rgba(255,255,255,.10)!important;color:#fff!important}
body.varex-dark .sidebar .nav a:active{background:rgba(255,255,255,.14)!important;border-bottom-width:1px!important}
...
body.varex-dark .sidebar .nav a.active .nav-label{color:#172554!important}
...
border-right:1px solid #0d193b!important;border-bottom:3px solid #08132f!important;
box-shadow:none!important
}
...
body.varex-dark .varex-logout-button:hover{background:#f1f5f9!important;color:#172554!important}
body.varex-dark .varex-logout-button:active{background:#e2e8f0!important;border-bottom-width:1px!important}
body.varex-dark .varex-power-icon{background:#f8fafc!important;color:#172554!important;border-color:#172554!important}

body.varex-dark hr{border-color:#29415f!important}
body.varex-dark ::-webkit-scrollbar{width:8px;height:8px}
body.varex-dark ::-webkit-scrollbar-track{background:#0b192d}
body.varex-dark ::-webkit-scrollbar-thumb{background:#324b69;border-radius:10px}
body.varex-dark ::-webkit-scrollbar-thumb:hover{background:#45617f}
