...
const id=s?.user?.id||this.getDeviceOwner?.()?.ownerId||"guest";
return String(id).replace(/[^a-zA-Z0-9_-]/g,"_")
},

scopeKey(k){
const scoped=new Set([this.keys.products,this.keys.sales,this.keys.customers,this.keys.suppliers,this.keys.employees,this.keys.transactions,this.keys.settings,this.keys.heldSales,this.keys.staffUsers,this.keys.subscription]);
return scoped.has(k)?`${k}__${this.getAccountScope()}`:k
},

getData(k){try{const x=JSON.parse(localStorage.getItem(this.scopeKey(k))||"[]");return Array.isArray(x)?x:[]}catch(e){return[]}},
saveData(k,d){try{localStorage.setItem(this.scopeKey(k),JSON.stringify(Array.isArray(d)?d:[]));return true}catch(e){return false}},
...
saveObject(k,d){try{localStorage.setItem(this.scopeKey(k),JSON.stringify(d||{}));return true}catch(e){return false}},
...
if((s.expires_at||0)<=Math.floor(Date.now()/1000)+60){
...
const r=await fetch(this.config.supabaseUrl+"/rest/v1/"+path,{...opt,headers:h});
...
if(m.includes("already registered")||m.includes("user already registered"))return"هذا البريد الإلكتروني مرتبط بحساب موجود مسبقاً. يرجى تسجيل الدخول بدلاً من إنشاء حساب جديد.";
...
if(m.includes("rate")||m.includes("security purposes"))return"تم إجراء محاولات كثيرة خلال وقت قصير. يرجى الانتظار قليلاً ثم المحاولة من جديد.";
...
const u=this.getSafeUser(user)||user||{},x={authorized:true,ownerId:u.id||"",name:u.name||"مالك المنشأة",username:u.username||"owner",email:this.normalizeEmail(u.email||""),authorizedAt:this.now()};
localStorage.setItem(this.keys.deviceAuth,"true");localStorage.setItem(this.keys.deviceOwner,JSON.stringify(x));return x
},
...
this.removeDeviceAuthorization();sessionStorage.removeItem(this.keys.session);localStorage.removeItem(this.keys.session);localStorage.removeItem(this.keys.cachedUser);localStorage.removeItem(this.keys.businessId);sessionStorage.removeItem("varex_authenticated");localStorage.removeItem("varex_authenticated");
...
redirectLoggedUser(){if((this.isLoginPage()||this.isRegisterPage())&&(this.isLoggedIn()||this.isDeviceAuthorized())){location.replace("./index.html");return true}return false},

async requestPasswordReset(email){
try{
const redirect=encodeURIComponent("https://varexapp.com/reset-password.html");
await this.authFetch("/auth/v1/recover?redirect_to="+redirect,{method:"POST",body:JSON.stringify({email:this.normalizeEmail(email)})});
...
const body={},data={};if(c.email!==undefined)body.email=this.normalizeEmail(c.email);["name","username","role"].forEach(k=>{if(c[k]!==undefined)data[k]=this.cleanText(c[k])});if(Object.keys(data).length)body.data={...(s.user.user_metadata||{}),...data};
...
if(String(n||"").length<8)return{success:false,message:"كلمة المرور الجديدة يجب أن تحتوي على 8 أحرف على الأقل."};
try{const s=this.getSession();if(!s)return{success:false,message:"يجب تسجيل الدخول بحساب المالك أولاً."};const u=await this.authFetch("/auth/v1/user",{method:"PUT",body:JSON.stringify({password:String(n)})});s.user=u;this.storeSession(s,true);return{success:true,message:"تم تغيير كلمة المرور بنجاح."}}catch(e){return{success:false,message:this.mapAuthError(e)}}
},

/* =========================================================
BUSINESS ID
========================================================= */
clearBusinessIdCache(){localStorage.removeItem(this.keys.businessId)},
async getCurrentBusinessId(force=false){
const session=this.getSession();
if(!session?.user?.id)throw new Error("لا توجد جلسة مستخدم صالحة.");
const cacheKey=`${this.keys.businessId}__${session.user.id}`;
if(!force){
const cached=localStorage.getItem(cacheKey);
if(cached)return cached
}
const uid=encodeURIComponent(session.user.id);
const rows=await this.dbFetch(`users?select=business_id&auth_user_id=eq.${uid}&limit=1`);
const businessId=Array.isArray(rows)&&rows[0]?.business_id?rows[0].business_id:null;
...
try{
const rows=await this.dbFetch("suppliers?select=id,name&order=name.asc");
const map={};(Array.isArray(rows)?rows:[]).forEach(x=>{if(x?.id)map[x.id]=x.name||""});return map
}catch(e){console.warn("VAREX SUPPLIERS MAP:",e);return{}}
},

async resolveSupplierIdByName(name){
const n=this.cleanText(name);if(!n)return null;
const rows=await this.dbFetch("suppliers?select=id,name&name=eq."+encodeURIComponent(n)+"&limit=1");
return Array.isArray(rows)&&rows[0]?.id?rows[0].id:null
},

async syncProductsFromSupabase(){
const [rows,supplierMap]=await Promise.all([this.dbFetch("products?select=*&order=created_at.asc"),this.getSupplierMap()]);
const products=(Array.isArray(rows)?rows:[]).map(x=>this.mapDbProduct(x,supplierMap)).filter(Boolean);
this.saveProducts(products);return products
},
...
const rows=await this.dbFetch("products?select=*",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify(body)});
await this.syncProductsFromSupabase();
return Array.isArray(rows)&&rows[0]?this.mapDbProduct(rows[0],await this.getSupplierMap()):null
},

async updateProductRemote(id,c={}){
if(!id)throw new Error("معرّف المنتج غير موجود.");
const body={updated_at:this.now()};
if(c.name!==undefined)body.name=this.cleanText(c.name);
if(c.sku!==undefined)body.sku=this.cleanText(c.sku)||null;
if(c.barcode!==undefined)body.barcode=this.cleanText(c.barcode)||null;
if(c.category!==undefined)body.category=this.cleanText(c.category)||null;
if(c.cost!==undefined)body.cost=this.positiveNumber(c.cost);
if(c.price!==undefined)body.price=this.positiveNumber(c.price);
if(c.quantity!==undefined)body.quantity=this.positiveNumber(c.quantity);
if(c.minimumStock!==undefined)body.minimum_stock=this.positiveNumber(c.minimumStock);
if(c.unit!==undefined)body.unit=this.cleanText(c.unit)||"piece";
if(c.status!==undefined)body.status=this.cleanText(c.status)||"active";
if(c.description!==undefined)body.description=this.cleanText(c.description)||null;
if(c.taxRate!==undefined)body.tax_rate=this.positiveNumber(c.taxRate);
...
await this.dbFetch("products?id=eq."+encodeURIComponent(id),{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(body)});
await this.syncProductsFromSupabase();return this.getProductById(id)
},

async deleteProductRemote(id){
if(!id)return false;
await this.dbFetch("products?id=eq."+encodeURIComponent(id),{method:"DELETE",headers:{Prefer:"return=minimal"}});
await this.syncProductsFromSupabase();return true
},

async adjustStockRemote(id,change){
const product=this.getProductById(id)||((await this.syncProductsFromSupabase()).find(x=>String(x.id)===String(id)));
if(!product)throw new Error("المنتج غير موجود.");
const next=this.toNumber(product.quantity)+this.toNumber(change);
if(next<0)throw new Error("الكمية المتوفرة غير كافية.");
await this.dbFetch("products?id=eq."+encodeURIComponent(id),{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({quantity:next,updated_at:this.now()})});
await this.syncProductsFromSupabase();return this.getProductById(id)
},

/* =========================================================
LOCAL/CACHE BUSINESS DATA
========================================================= */
...
completeSale(s={}){
const items=Array.isArray(s.items)?s.items:[];if(!items.length)return{success:false,message:"لا توجد منتجات في الفاتورة."};const p=this.getProducts();
...
initialize(){
[this.keys.products,this.keys.sales,this.keys.customers,this.keys.suppliers,this.keys.employees,this.keys.transactions,this.keys.heldSales].forEach(k=>{const sk=this.scopeKey(k);if(localStorage.getItem(sk)===null)localStorage.setItem(sk,"[]")});
return true
}
};
...
varexInstallSharedStyles();varexInstallStaffUI();varexApplyTheme(varexGetTheme());varexBuildMenu();varexInstallTopSwitchUserButton();varexInitStaffAccess();varexBuildMenu();varexInstallTopSwitchUserButton();varexShowCurrentUser()
}
